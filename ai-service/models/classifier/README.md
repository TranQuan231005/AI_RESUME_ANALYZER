...
- **Out-of-Distribution Rejection:** Unknown threshold is calibrated on in-domain validation plus 50 OOD validation samples.
...
- `evaluation/reports/classification.md`: Independent in-domain/OOD test results and six-label confusion matrix.

## Reproduction Command
To retrain and evaluate the model from scratch:
```bash
# 1. Train candidate models and serialize winning pipeline
python3.11 ai-service/training/train_classifier.py

# 2. Evaluate against independent held-out test split
python3.11 evaluation/scripts/evaluate_classifier.py --output evaluation/reports/classification.md
```
